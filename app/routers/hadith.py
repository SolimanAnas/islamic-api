...
def search_hadith(
    ...
    store = get_store()
    fields = ["english", "text", "arabic"] if lang == "en" else ["arabic"]
    all_hadith = []
...
def get_hadith(collection: str, hadith_id: int):
    ...
    for h in col["hadiths"]:
        if (h.get("hadithNumber") == hadith_id or
            h.get("hadithnumber") == hadith_id or
            h.get("idInBook") == hadith_id or
            h.get("id") == hadith_id or
            h.get("hadithNumber") == str(hadith_id)):
            return {"success": True, "data": h}
